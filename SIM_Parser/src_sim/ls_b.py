...
def get_LSB_report(name):
    ...
    with open(name, "r", encoding="cp1252", errors="ignore") as f:
        ...
        for line in lsc_rpt:
            ...
            if ('WALL CONDUCTION' in line or 'ROOF CONDUCTION' in line or
                'WINDOW GLASS+FRM COND' in line or 'WINDOW GLASS SOLAR' in line or 'DOOR CONDUCTION' in line or
                'INTERNAL SURFACE COND' in line or 'UNDERGROUND SURF COND' in line 
                # or 'LIGHT     TO SPACE' in line or 'EQUIPMENT TO SPACE' in line or 'PROCESS   TO SPACE'
                or ('SPACE' in line and '/' not in line and '.' not in line)  and '/' not in line):
                lsc_str.append(line)
        ...
        df = lsc_df[~lsc_df.astype(str).apply(lambda x: x.str.contains(r'\*', na=False)).any(axis=1)]
        # 1️⃣ Identify rows containing 'SPACE' in any column
        def merge_row_if_space(row):
            row_str = row.dropna().astype(str)   # 🔹 remove None/NaN first
            if row_str.str.contains('SPACE', case=False, na=False).any():
                merged = ' '.join(row_str)       # 🔹 only real values
                new_row = [np.nan] * len(row)
                new_row[0] = merged
                return new_row
            return row

        df = df.apply(merge_row_if_space, axis=1, result_type='expand')
        df = df.iloc[:, :-4]
        ...
        for _, row in df.iterrows():
            ...
            # SPACE row
            if label.upper().startswith('SPACE'):
                # save previous space
                if current_row:
                    records.append(current_row)

                # start new space
                current_space = label.replace('SPACE', '').strip()
                current_row = {'Space': current_space}
        ...
        lsc_df = pd.DataFrame(records)

        # optional: column order
        # cols = ['Space'] + [c for c in lsc_df.columns if c != 'Space']
        # lsc_df = lsc_df[cols]
        # st.write(lsc_df)
