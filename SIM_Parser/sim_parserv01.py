...
def get_report_and_save(report_function, sim_path, file_suffix, title_container, status_container):
    ...
    try:
        ...
        st.session_state[key].append(
            f"<span style='color:green; font-weight:600; margin-right:14px;'>✅ {file_suffix}</span>"
        )

    except Exception:
        # add error badge
        st.session_state[key].append(f"<span style='color:red; font-weight:600; margin-right:14px;'>❌ {file_suffix}</span>")
        temp_file_path = None

    # -------- UI Render --------
    title_container.markdown("<b style='color:green; font-weight:600; font-size:1200;'>SIM Reports:</b>",unsafe_allow_html=True)
...
def main(uploaded_file):
    if uploaded_file is not None:
        ...
        report_functions = [
            ...
            (ls_c.get_LSC_losses_report, 'LSC_LOSS.csv', 'LS-C_LOSS'),
            (lv_d.get_LVD_report, 'LVD.csv', 'LV-D'),
            (lvd_summary.get_LVD_Summary_report, 'LVD_Summary.csv', 'LV-D_Summary'),
            (pv_a.get_PVA_report, 'PVA.csv', 'PV-A'),
            (sv_a.get_SVA_report, 'SVA.csv', 'SV-A'),
            (sva_zone.get_SVA_Zone_report, 'SVA_Zone.csv', 'SV-A_Zone'),
        ...
        if download_files:
            st.markdown(f"<br><span style='color:green;'>✅ SIM Parsed Successfully!!</span>", unsafe_allow_html=True)
